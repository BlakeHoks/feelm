...
export function MovieCard({ movie, overlay, className }: MovieCardProps) {
  ...
  return (
    <article
      className={cn(
        'group relative flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-bg-elevated',
      ...
      >
        <div className="relative flex aspect-[2/3] items-center justify-center overflow-hidden bg-gradient-to-br from-surface to-bg-deep">
          {poster ? (
            ...
            <span className="font-mono text-fg-subtle text-xs">[ no poster ]</span>
          )}
        </div>
        <div className="flex flex-1 flex-col p-4">
          <h3 className="mb-1 line-clamp-1 font-semibold text-base text-fg">{movie.title}</h3>
          ...
          {movie.overview && (
            <p className="line-clamp-3 text-fg-muted text-sm leading-relaxed">{movie.overview}</p>
          )}
        </div>
      </Link>
