...
* Нет обязательных `index.ts` в каждой папке для реэкспорта
* Импорты идут напрямую к файлу компонента/хука
* Сохраняется деление по слоям: смысл и ответственность каждого слоя остаётся из FSD
